...
static void txtViewer_meun_open(void)
{
	...
	setPrevMenuOpen(&prevMenuData, txtViewer_meun_open);//多次进入这个菜单，上级菜单也不会被覆盖
	buttons_setFuncs(menu_up, menu_select, menu_down);
	display_setDrawFunc(menu_draw);  //绑定绘制函数为menu_draw
	setMenuInfo(24, MENU_TYPE_STR, PSTR("Viewer"));   //获取当前菜单信息（选项个数，菜单类型是文字还是图标）
